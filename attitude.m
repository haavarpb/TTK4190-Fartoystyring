...
%% USER INPUTS
h = 0.1;                     % sample time (s)
N  = 8000;                    % number of samples. Should be adjusted
...
table = zeros(N+1,18);        % memory allocation
...
N_array = 1:(N + 1);
phi_d = 0;
theta_d = 15*cosd(0.1*N_array);
psi_d = 10*sind(0.05*N_array);

T_f2b = @(phi, theta, psi) [1 0 -sind(theta); 0 cosd(phi) cosd(theta)*sind(phi); 0 -sind(phi) cosd(theta)*cosd(phi)];
...
for i = 1:N+1
   ...
   t = (i-1)*h;                  % time
   tau = - K_d*w - k_p*e_tilde;       % control law
   ...
   table(i,:) = [t q' phi theta psi w' tau' q_tilde'];  % store data in table
